#' Build a convergence-based stop_FUN and attach it to a Game object
#'
...
#'
#' @export
build_stopFUN_convergence <- function(G,
                                      watching_field,
  ...
  if(!(watching_field %in% existing_fields)){
    stop("'watching_field' (\"", watching_field, "\") is not a field of 'G'. ",
         "Add it first (e.g. via State() or Active()) before calling build_stopFUN_convergence().")
  }
...
#'
#' Internal helper used by \code{\link{build_stopFUN_convergence}} (and
#' potentially other \code{build_*} generators) to splice an additional
...
#'   modified in place.
#' @seealso \code{\link{build_stopFUN_convergence}}
#' @keywords internal
