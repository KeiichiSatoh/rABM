...
test_that("modify_G: add_stage works", {
  G <- setABM(agents = 2, stage = data.frame(age = c(1,2)))
  G2 <- modify_G(G, "stage2", method = "add_stage", field = matrix(0,2,2))
...
test_that("modify_G: add_global_FUN works", {
  ...
  G2 <- modify_G(G, "global_FUN_added", method = "add_global_FUN", field = global_FUN)
...
test_that("modify_G: add_select_FUN works", {
  ...
  G2 <- modify_G(G, "select_FUN_added", method = "add_select_FUN", field = select_FUN)
...
test_that("modify_G: add_stop_FUN works", {
  ...
  G2 <- modify_G(G, "stop_FUN_added", method = "add_stop_FUN", field = stop_FUN)
...
test_that("modify_G: add_update_FUN works", {
  ...
  G2 <- modify_G(G, "update_FUN_added", method = "add_update_FUN", field = update_FUN)
...
test_that("modify_G: add_active_binding works", {
  ...
  G2 <- modify_G(G, "active_binding_added", method = "add_active_binding", field = active_binding)
...
test_that("modify_G: replaces a field without setting a new name.", {
  G <- setABM(agents = 2, stage = data.frame(age = c(1,2)))
  G2 <- modify_G(G, "stage", method = "replace", field = matrix(0,2,2))
  expect_false(identical(G2$stage2, G$stage))
})

test_that("modify_G: replaces and renames a field", {
  G <- setABM(agents = 2, stage = data.frame(age = c(1,2)))
  G2 <- modify_G(G, "stage", method = "replace", field = matrix(0,2,2), new_field_name = "stage2")
  expect_true("stage2" %in% ls(G2))
  expect_false("stage" %in% ls(G2))
  expect_false(identical(G2$stage2, G$stage))
})
